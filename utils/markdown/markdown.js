...
const md = new MarkdownIt({
  ...
  typographer: true,
  quotes: '“”‘’'
}).use(emojiPlugin).use(checkboxPlugin, {
  divWrap: true,
...
// code_inline解析
const codeInlineRenderer = md.renderer.rules.code_inline ||
...
// ordered_list_open
const orderedListRenderer = md.renderer.rules.ordered_list_open ||
  function (tokens, idx, options, env, self) {
    return self.renderToken(tokens, idx, options)
  }
...
// bullet_list_open
const bulletListRenderer = md.renderer.rules.ordered_list_open ||
  function (tokens, idx, options, env, self) {
    return self.renderToken(tokens, idx, options)
  }
...
// blockquote_open
const blockquoteRenderer = md.renderer.rules.blockquote_open ||
  function (tokens, idx, options, env, self) {
    return self.renderToken(tokens, idx, options)
  }
...
// table
const tableOpenRenderer = md.renderer.rules.table_open ||
  function (tokens, idx, options, env, self) {
    return self.renderToken(tokens, idx, options)
  }
...
const tableCloseRenderer = md.renderer.rules.table_close ||
  function (tokens, idx, options, env, self) {
    return self.renderToken(tokens, idx, options)
  }
...
// code-block
const codeBlockRenderer = md.renderer.rules.code_block ||
  function (tokens, idx, options, env, self) {
    return self.renderToken(tokens, idx, options)
  }
